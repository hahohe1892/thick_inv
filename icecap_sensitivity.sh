...
xs = [5.0, 10.0, 15.0, 25.0, 50.0]
for i in xs:
    bed = get_nc_data('icecap_output_S_ref_rand_{}_correct_diffusivity.nc'.format(i), 'topg', -1)
    beds.append(bed)
    surf = get_nc_data('icecap_output_S_ref_rand_{}_correct_diffusivity.nc'.format(i), 'usurf', -1)
    surfs.append(surf)
    surfs_in.append(np.maximum(true_surf + np.random.normal(0, x, np.shape(true_surf)), true_bed))
    vols.append(np.sum(surf-bed))
    taucs.append(np.nanmedian(get_nc_data('icecap_output_S_ref_rand_{}_correct_diffusivity.nc'.format(i), 'tauc', 0)[mask==1]))
    vels.append(np.nanmean(get_nc_data('icecap_output_S_ref_rand_{}_correct_diffusivity.nc'.format(i), 'velbar_mag', 0)[mask==1]))
    fluxes.append(np.sum(get_nc_data('icecap_output_S_ref_rand_{}_correct_diffusivity.nc'.format(i), 'flux_mag', 0)[mask==1]))
